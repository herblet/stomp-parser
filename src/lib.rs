...
impl StompParseError {
    fn new<S: Into<String>>(message: S) -> StompParseError {
        StompParseError {
...
fn always_fail<'a, O, E: FullError<&'a [u8], StompParseError>>(
    input: &'a [u8],
) -> IResult<&[u8], O, E> {
    Err(Err::Failure(E::from_error_kind(input, ErrorKind::Tag)))
}

fn null<'a, E: 'a + FullError<&'a [u8], StompParseError>>(
    input: &'a [u8],
) -> IResult<&[u8], &'a [u8], E> {
    terminated(
...
fn command_line<'a, E: FullError<&'a [u8], E2>, E2>(
    input: &'a [u8],
) -> IResult<&[u8], &'a [u8], E> {
    terminated(is_not("\r\n"), line_ending)(input)
}

fn remaining_without_null<'a, E: FullError<&'a [u8], StompParseError>>(
    input: &'a [u8],
) -> IResult<&'a [u8], &'a [u8], E> {
...
#[cfg(test)]
mod tests {
    use crate::model::frames::client::parsers::client_frame;
    use crate::model::frames::client::ClientFrame;
    use crate::model::{AckType, HeaderValue, HeartBeatIntervalls, StompVersion, StompVersions};
    ...
    fn CONNECT_frame_rejects_body() {
        let frame = client_frame(b"CONNECT\naccept-version:a\nhost:b\r\n\nfoobar\x00".to_vec());
        assert!(frame.is_err());
    }
    ...
    fn it_doesnt_match_rubbish() {
        let frame = client_frame(b"coNct\naccept-version:a\nhost:b\r\n\nasakldkf\x00".to_vec());
        assert!(frame.is_err());
    }
    ...
    fn STOMP_frame_works() {
        let frame = client_frame(b"STOMP\naccept-version:a\nhost:b\r\n\n\x00".to_vec()).unwrap();
        assert!(matches!(frame, ClientFrame::Stomp(_)));
    }
    ...
    fn SUBSCRIBE_frame_requires_id() {
        let frame = client_frame(b"SUBSCRIBE\r\ndestination:y/b\nack:client\n\n\x00".to_vec());
        assert!(frame.is_err())
    }
